...
class ConnectorController extends Controller
{
    ...
    public function getUsageStatus(Request $request)
    {
        ...
        $status = \App\Services\Core\UsageLimitService::getUsageStatus($orgId);
    ...
    public function startIngest(Request $request, string $id)
    {
        ...
        $docLimit = \App\Services\Core\UsageLimitService::canAddDocument($orgId);
        if (!$docLimit['allowed']) {
            return response()->json([
    ...
    public function disconnect(Request $request, string $id)
    {
        ...
        try {
            ...
            foreach (array_chunk($targets, 50) as $chunk) {
                \App\Jobs\Core\DeleteCloudinaryAssetsJob::dispatch($chunk)->onQueue('default');
            }
        ...
        if (!empty($chunkIds)) {
            try {
                $vectorStore = new \App\Services\Core\VectorStoreService();
