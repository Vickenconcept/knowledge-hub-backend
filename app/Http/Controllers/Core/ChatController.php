...
class ChatController extends Controller
{
    public function ask(Request $request, EmbeddingService $embeddingService, VectorStoreService $vectorStore, RAGService $rag)
    {
        ...
        $chatLimit = \App\Services\Core\UsageLimitService::canChat($orgId);
        ...
        $spendLimit = \App\Services\Core\UsageLimitService::isWithinSpendLimit($orgId);
        ...
        try {
            ...
            $routing = \App\Services\Core\ContextRouter::routeQuery($queryText, $conversationHistory);
            
            // Handle SESSION MEMORY QUERIES (last week, previous conversations)
            if (\App\Services\Core\ConversationMemoryService::isSessionMemoryQuery($queryText)) {
                Log::info('Session memory query detected', ['query' => $queryText]);
                
                $sessionMemory = new \App\Services\Core\SessionMemoryService();
            ...
            $entityInfo = \App\Services\Core\EntitySearchService::isEntityQuery($queryText);
            ...
            if ($entityInfo['is_entity_query']) {
                ...
                $entityResults = \App\Services\Core\EntitySearchService::searchEntities($queryText, $entityInfo, $orgId);
                $formattedResponse = \App\Services\Core\EntitySearchService::formatEntityResults($entityResults, $queryText, $entityInfo['is_count_query']);
            ...
            if ($routing['route_type'] === 'meta') {
                $metaResponse = \App\Services\Core\ConversationMemoryService::buildMetaResponse($queryText, $conversationHistory);
            ...
            $styleInference = \App\Services\Core\StyleInferenceService::detectExplicitStyleRequest($queryText);
            ...
            if ($styleInference && $styleInference['detected']) {
                ...
                $responseStyle = \App\Services\Core\StyleInferenceService::getRecommendedStyle($queryText, $currentStyle);
            ...
            if (self::isPersonQuery($queryText)) {
                $nameMatchingResult = \App\Services\Core\NameMatchingService::findNameMatches($queryText, $snippets, $orgId, $user->id);
                
                // If no exact match found, provide intelligent response
                if ($nameMatchingResult['no_matches'] || $nameMatchingResult['confidence'] < 0.5) {
                    $intelligentResponse = \App\Services\Core\NameMatchingService::generateIntelligentResponse($nameMatchingResult, $queryText);
            ...
            $enhancedContext = \App\Services\Core\ContextRouter::buildEnhancedContext($routing, $conversation->id, $conversationHistory);
            ...
            $styleConfig = \App\Services\Core\ResponseStyleService::getStyleInstructions($responseStyle);
    ...
    {
        $styles = \App\Services\Core\ResponseStyleService::getAvailableStyles();
