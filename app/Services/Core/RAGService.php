...
class RAGService
{
    ...
    ): string
    {
        ...
        $intentService = new \App\Services\Core\QueryIntentService();
        ...
        $confidenceService = new \App\Services\Core\ConfidenceScoreService();
        ...
        if (!empty($conversationContext)) {
            $buf .= \App\Services\Core\ConversationMemoryService::formatConversationForPrompt($conversationContext, 3);
        }
        ...
        $stylePrompt = \App\Services\Core\ResponseStyleService::buildStylePrompt($responseStyle);
        $buf .= $stylePrompt;
    ...
    public function callLLM(string $prompt, int $maxTokens = 1500, ?string $orgId = null, ?int $userId = null, ?string $conversationId = null, ?string $queryText = null): array
    {
        ...
        if ($orgId) {
            ...
            if ($tokensInput > 0 || $tokensOutput > 0) {
                \App\Services\Core\CostTrackingService::trackChat(
                    $orgId,
                    $userId,
                    $this->chatModel,
                    $tokensInput,
