...
use App\Services\Core\GoogleDriveService;
use App\Services\Core\DropboxService;
use App\Services\Core\FileUploadService;
use App\Services\Core\DocumentExtractionService;
use App\Services\Core\HttpDownloadService;
use App\Models\Document;
use App\Jobs\Core\ProcessLargeFileJob;
...
class IngestConnectorJob implements ShouldQueue
{
    ...
    {
        $extractor = new \App\Services\Core\DocumentExtractionService();
        ...
        foreach ($documents as $index => $document) {
            ...
            $quotaCheck = \App\Services\Core\UsageLimitService::canAddDocument($this->orgId);
            ...
            try {
                ...
                if (!$existingTracking) {
                    \App\Services\Core\CostTrackingService::trackDocumentIngestion(
                        $this->orgId,
                        $document->id,
                        $this->connectorId,
                        $job->id, // ingestJobId
    ...
    private function processNotionPages($connector, $tokens, $job, &$docs, &$chunks, &$errors, &$processedFiles, &$skippedFiles)
    {
        ...
        $notionService = new \App\Services\Core\NotionService();
        $extractor = new \App\Services\Core\DocumentExtractionService();
        $uploader = new \App\Services\Core\FileUploadService();
    ...
    private function processGoogleDriveFiles($connector, $tokens, $job, &$docs, &$chunks, &$errors, &$processedFiles, &$skippedFiles)
    {
        ...
        $driveService = new \App\Services\Core\GoogleDriveService();
        $extractor = new \App\Services\Core\DocumentExtractionService();
        $uploader = new \App\Services\Core\FileUploadService();
        ...
        try {
            ...
            $job->save();
                    ...
                    \App\Services\Core\CostTrackingService::trackFilePull(
                        $this->orgId,
                        'google_drive',
                        1,
                        strlen($content),
                    ...
                    $classifier = new \App\Services\Core\DocumentClassificationService();
    ...
    {
        try {
            $embeddingService = app(\App\Services\Core\EmbeddingService::class);
            $vectorStore = app(\App\Services\Core\VectorStoreService::class);
    ...
    private function processSlackMessages($connector, $tokens, $job, &$docs, &$chunks, &$errors): void
    {
        ...
        try {
            $slack = new \App\Services\Core\SlackService();
