...
class IngestConnectorJob implements ShouldQueue
{
    ...
    public function handle(): void
    {
        ...
        if ($connector->type === 'google_drive') {
            ...
            } elseif ($connector->type === 'confluence') {
                Log::info('Starting Confluence page ingestion...');
                $this->processConfluencePages($connector, $tokens, $job, $docs, $chunks, $errors, $processedFiles, $skippedFiles);
                Log::info('Confluence page ingestion completed', [
                    'docs' => $docs,
                    'chunks' => $chunks,
                    'errors' => $errors,
                    'processed' => $processedFiles,
    ...
    /**
     * Ingest Confluence pages for a given connector.
     *
     * Expected encrypted_tokens structure for Confluence connectors:
     * {
     ...
     * }
     */
    private function processConfluencePages($connector, $tokens, $job, &$docs, &$chunks, &$errors, &$processedFiles, &$skippedFiles)
    {
        Log::info('>>> processConfluencePages started', [
            'connector_id' => $connector->id,
            'has_base_url' => !empty($tokens['base_url']),
            'has_email' => !empty($tokens['email']),
            'has_api_token' => !empty($tokens['api_token']),
            'space_key' => $tokens['space_key'] ?? null,
        ]);

        if (empty($tokens['base_url']) || empty($tokens['email']) || empty($tokens['api_token']) || empty($tokens['space_key'])) {
            Log::error('Confluence connector is missing required credentials', [
                'connector_id' => $connector->id,
            ]);
            $errors++;
            return;
        ...
        try {
            ...
            do {
                $result = $confluence->listPages($spaceKey, $limit, $start);
                $pages = $result['results'] ?? [];
                $allPages = array_merge($allPages, $pages);
                $start = $result['next_start'] ?? null;
            } while ($start !== null);

            $totalFiles = count($allPages);

            Log::info("✅ Found {$totalFiles} Confluence pages", [
                'space' => $spaceKey,
                'page_count' => $totalFiles,
            ]);
            ...
            foreach ($allPages as $index => $pageSummary) {
                // Allow cancel mid-run
                $job->refresh();
                if ($job->status === 'cancelled') {
                    Log::info('🛑 Job cancelled by user during Confluence sync', [
                        'job_id' => $job->id,
                        'processed_files' => $processedFiles,
                        'total_files' => $totalFiles,
                    ]);
                ...
                $quotaCheck = \App\Services\UsageLimitService::canAddDocument($this->orgId);
                if (!$quotaCheck['allowed']) {
                    Log::warning('🛑 Document quota reached mid-Confluence sync, stopping', [
                        'job_id' => $job->id,
                        'quota_used' => $quotaCheck['current_usage'],
                        'quota_limit' => $quotaCheck['limit'],
                    ]);
                    ...
                    $job->stats = array_merge($job->stats, [
                        'quota_reached' => true,
                        'quota_message' => 'Document quota reached while syncing Confluence.',
                    ]);
                    $job->save();
                    break;
                }
                ...
                try {
                    ...
                    $pageUrl = $confluence->getPageUrl($page) ?? null;

                    Log::info("Processing Confluence page #{$index}", [
                        'title' => $title,
                        'id' => $pageId,
                        'url' => $pageUrl,
                    ]);
                    ...
                    // Check if document already exists in same scope
                    $existingDocument = \App\Models\Document::where('org_id', $this->orgId)
                        ->where('connector_id', $connector->id)
                        ->when($pageUrl, function ($q) use ($pageUrl) {
                            $q->where('source_url', $pageUrl);
                        })
                        ->where('source_scope', $connector->connection_scope)
                        ->first();

                    $text = $confluence->extractTextFromPage($page);

                    if ($text === '' || empty(trim($text))) {
                        Log::info("⚠️ No text extracted from Confluence page: " . $title);
                        $skippedFiles++;
                        $processedFiles++;
                        continue;
                    }
                    ...
                    if ($existingDocument) {
                        if ($existingDocument->sha256 === $contentHash) {
                            Log::info("⏭️ Confluence page unchanged (by hash), skipping: " . $title, [
                                'document_id' => $existingDocument->id,
                            ]);
                            $skippedFiles++;
                            $processedFiles++;
                        ...
                        Log::info("🔄 Confluence page changed, updating: " . $title, [
                            'document_id' => $existingDocument->id,
                        ]);
                        ...
                        $document = Document::create([
                            ...
                            'source' => 'confluence',
                            'source_url' => $pageUrl,
                            'source_scope' => $connector->connection_scope,
                            'sha256' => $contentHash,
                            'user_id' => $connector->user_id ?? null,
                            'workspace_name' => $connector->workspace_name,
                            'metadata' => [
                    ...
                    // Chunk and create embeddings via standard job
                    \App\Jobs\CreateChunksJob::dispatch($document->id, $this->orgId, $text);

                    // We don't know exact chunk count synchronously; increment approx by 1 for stats
                    $chunks += 1;
                    $processedFiles++;

                } catch (\Throwable $e) {
                    Log::error('Confluence page processing error', [
                        'error' => $e->getMessage(),
                        'page_index' => $index,
                        'space' => $spaceKey,
                    ]);
            ...
            Log::error('Confluence sync failed', [
                'error' => $e->getMessage(),
                'space' => $spaceKey,
            ]);
            $errors++;
